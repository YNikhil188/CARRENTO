...
if ($result && $result->num_rows > 0) {
    ...
    if (
        empty($row['name']) || empty($row['phone']) || empty($row['email']) || empty($row['aadhaar']) || empty($row['address']) || 
    ...
    <style>
      * {
      ...
      .booking-form input[type="date"]:focus {
        outline: none;
        border-color: #ff3b3b;
        box-shadow: 0 0 8px rgba(255, 39, 39, 0.5);
        background-color: #444;
      }

      /* Customize the date picker icon (pseudo-element) */
      .booking-form input[type="date"]::-webkit-calendar-picker-indicator {
        filter: invert(70%) sepia(100%) saturate(1000%) hue-rotate(330deg) brightness(90%);
        cursor: pointer;
        padding: 5px;
      }

      .booking-form input[type="date"]::-webkit-calendar-picker-indicator:hover {
        filter: invert(80%) sepia(100%) saturate(1500%) hue-rotate(330deg) brightness(100%);
      }
...
<body>
    ...
    <div class="booking-container">
        ...
        <div class="form-container">
            ...
            <form action="process_booking.php" method="POST" class="booking-form">
                ...
                <input type="date" name="start_date" id="start_date" min="<?php echo $today; ?>" required onchange="updateEndDateMin()">

                <label for="end_date">End Date:</label>
                <input type="date" name="end_date" id="end_date" min="<?php echo $today; ?>" required onchange="calculateTotalDays()">
    ...
    <script>
        function updateEndDateMin() {
            ...
            if (startDate) {
                endDateInput.min = startDate; // Set min of end date to start date
                if (endDateInput.value < startDate) {
                    endDateInput.value = ''; // Reset end date if it's before start date
                }
            }
        }
        ...
        function calculateTotalDays() {
            ...
            if (startDate && endDate) {
                ...
                if (diffDays >= 1) {
                    ...
                    document.getElementById('end_date').value = ''; // Reset end date if invalid
                }
            }
        }
    </script>
